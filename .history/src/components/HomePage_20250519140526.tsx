...
export function HomePage() {
  ...
  useEffect(() => {
    async function fetchAllListings() {
      try {
        setIsLoading(true);
        console.log("Initial fetch of all listings...");
        ...
        if (error) {
          ...
          if (mappedProducts.length > 0) {
            ...
            console.log("No products found, using static listings");
            setListings(staticListings);
          }
        }
      } catch (err) {
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {categories.slice(0, 5).map(category => (
                    <button 
                      key={category.id}
                      onClick={() => handleCategorySelect(category.id)}
                      className={`px-3 py-1 ${selectedCategory === category.id ? 
                        'bg-blue-100 text-blue-700 border border-blue-300' : 
                        'bg-gray-100 text-gray-700 hover:bg-gray-200'} rounded-full text-sm transition`}
                    >
                      {t(`categories.${category.name}`)}
                    </button>
                  ))}
