...
export function HomePage() {
  ...
  useEffect(() => {
    async function fetchListings() {
      try {
        ...
        // First fetch all products to see what's available
        ...
        if (allError) {
          ...
          console.log("Available categories in database:", 
            allProducts.map(p => p.category).filter((v, i, a) => a.indexOf(v) === i));
        }
        ...
        if (error) {
          ...
          setListings(mappedProducts.length > 0 ? mappedProducts : staticListings);
          setError(null);
        }
      } catch (err) {
  ...
  const filteredListings = useMemo(() => {
    return listings.filter((listing) => {
      ...
      if (selectedCategory) {
        ...
        if (categoryObj) {
          // If this is a subcategory selection
          if (selectedSubcategory) {
            matchesCategory = listing.category === selectedSubcategory;
          } else {
            // Check if the listing belongs to any subcategory of the selected category
            matchesCategory = categoryObj.subcategories.some(sub => 
              sub.id === listing.category
            );
          }
        } else {
  ...
  }, [searchQuery, selectedCategory, selectedSubcategory, listings, categories]);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="flex-grow">
        ...
        {/* Main Listings Grid - shows filtered results when a category is selected */}
