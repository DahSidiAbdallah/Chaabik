        ...
        <section className="py-6 xs:py-8 sm:py-12">
          <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
            <div className="flex gap-4 mb-6">
              <button
                className={`px-4 py-2 rounded-lg font-semibold border ${selectedCategory === null ? 'bg-yellow-400 text-white' : 'bg-white text-gray-700'}`}
                onClick={() => { setSelectedCategory(null); setSearchBarCategory(null); navigate('/'); }}
              >
                All
              </button>
              <button
                className={`px-4 py-2 rounded-lg font-semibold border ${selectedCategory === 'lost' ? 'bg-yellow-400 text-white' : 'bg-white text-gray-700'}`}
                onClick={() => { setSelectedCategory('lost'); setSearchBarCategory('lost'); navigate('/?category=lost'); }}
              >
                Lost
              </button>
              <button
                className={`px-4 py-2 rounded-lg font-semibold border ${selectedCategory === 'found' ? 'bg-yellow-400 text-white' : 'bg-white text-gray-700'}`}
                onClick={() => { setSelectedCategory('found'); setSearchBarCategory('found'); navigate('/?category=found'); }}
              >
                Found
              </button>
            </div>
            <h2 className="text-xl xs:text-2xl font-bold text-gray-900 mb-4 xs:mb-6 sm:mb-8">
              {selectedCategory ? 
            ...
            {isSearching ? (
              ...
              <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 xs:gap-4 sm:gap-5 lg:gap-6">
