...
export function HomePage() {
  ...
  // Fetch listings from Supabase with improved error handling
  useEffect(() => {
    async function fetchListings() {
      try {
        ...
        if (tablesError) {
          console.error('Error connecting to listings table:', tablesError);
          setDebugInfo(`Table connection error: ${tablesError.message}`);
          throw new Error(`Cannot connect to listings table: ${tablesError.message}`);
        }
        
        console.log("Successfully connected to listings table");
        
        // Now let's try a simple query first
        ...
        console.log("Simple query successful, got", simpleData?.length, "records");
        
        // Now try with the full query including joins
        let query = supabase
          .from('listings')
          .select(`
            id, 
        ...
        // Only add status filter if there are records with that column
        const hasStatusColumn = simpleData?.[0]?.status !== undefined;
        if (hasStatusColumn) {
        ...
        if (error) {
          console.error('Error fetching listings with order:', error);
          setDebugInfo(`Order query error: ${error.message}`);
          throw error;
        }

        console.log('Successfully fetched', data?.length, 'listings');
        
        // Let's also log the structure of the first listing to debug
        if (data && data.length > 0) {
          console.log('Sample listing structure:', JSON.stringify(data[0], null, 2));
        }
  ...
  const filteredListings = useMemo(() => {
    return listings.filter((listing) => {
      ...
      const matchesCategory = !selectedSubcategory || listing.category_id === selectedSubcategory;
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {filteredListings.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredListings.map((listing) => (
                  <ListingCard
                    ...
                    seller_profile={listing.profiles ? {
              ...
              <div className="text-center py-12">
                <p className="text-gray-500">{searchQuery ? t('search.noResults') : 
                  error ? 'Please check console for debugging information' : t('listings.noListings')}
                </p>
                {!error && listings.length === 0 && (
                  <p className="mt-2 text-sm text-gray-400">
                    No listings found in the database. Create your first listing!
                  </p>
                )}
              </div>
            )}
