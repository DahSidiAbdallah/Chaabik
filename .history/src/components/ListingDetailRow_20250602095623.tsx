...
export function ListingDetailRow({
  ...
  return (
    <Link to={`/product/${id}`} className="block group mb-3 sm:mb-4 w-full focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-opacity-50 rounded-lg">
      <div className={`relative bg-white rounded-lg overflow-hidden transition-all duration-300 hover:shadow-xl border border-gray-200 flex flex-col sm:flex-row ${is_sold ? 'opacity-60 filter grayscale-[60%]' : ''}`}>
        {/* Image Container */}
        <div className="w-full h-52 sm:w-40 sm:h-auto md:w-48 flex-shrink-0 bg-gray-100 group-hover:opacity-95 transition-opacity">
          <img
            src={imageUrl}
            alt={title}
        ...
        {/* Sold Overlay - full card coverage if needed, or keep as is if only for image */}
        {is_sold && (
