  ...
  return (
    <Link to={`/product/${id}`} className="block group mb-3 sm:mb-4 w-full focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-opacity-50 rounded-lg">
      <div className={`relative bg-white rounded-lg overflow-hidden transition-all duration-300 hover:shadow-xl border border-gray-200 flex flex-col sm:flex-row sm:h-36 md:h-40 ${is_sold ? 'opacity-60 filter grayscale-[60%]' : ''}`}>
        ...
        {/* Details Container - Make it relative for absolute positioning of children on sm+ */}
        <div className="p-3 sm:p-4 flex-grow flex flex-col w-full overflow-hidden sm:h-full sm:relative">
          ...
          {/* Location and Condition (normal flow) */}
          <div className="text-xs text-gray-500 mt-1 sm:mt-2 flex-shrink-0">
          ...
          <span className="hidden sm:group-hover:flex absolute bottom-3 right-3 sm:bottom-4 sm:right-4 items-center text-blue-600 font-semibold whitespace-nowrap text-sm p-1 bg-white/80 backdrop-blur-sm rounded-md">
            {t('common.viewDetails')}
            <ChevronRight className="w-4 h-4 ml-0.5" />
          </span>
          {/* Fallback View Details for mobile (always visible, normal flow, or styled differently) */}
          <div className="sm:hidden mt-auto pt-2 border-t border-gray-100 text-center">
             <span className="flex items-center justify-center text-blue-600 font-semibold text-sm py-1">
                {t('common.viewDetails')}
                <ChevronRight className="w-4 h-4 ml-0.5" />
            </span>
          </div>
