...
import { MapPin, Star, Calendar, ShoppingBag, CheckCircle, Award, BarChart } from 'lucide-react';
        ...
        <div className="p-2 xs:p-3 sm:p-4 flex-grow flex flex-col">
          ...
          <div className="flex items-center mb-2">
            <div className="flex mr-1">
              {[...Array(5)].map((_, i) => (
                <Star 
                  key={i} 
                  className={`w-3 h-3 ${i < Math.round(seller.rating) ? 'text-yellow-400' : 'text-gray-200'}`} 
          ...
          <div className="flex items-center justify-between text-xs text-gray-500 border-t pt-2 mt-auto border-gray-100">
            <div className="flex items-center">
              <span className="text-gray-500 mr-1 hidden xs:inline">{t('product.condition')}:</span>
              <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium ${
                condition === 'New' ? 'bg-green-100 text-green-800' :
                condition === 'Like New' ? 'bg-teal-100 text-teal-800' :
                ...
                {condition === 'New' && <CheckCircle className="w-3 h-3 mr-1" />}
                {condition === 'Like New' && <Award className="w-3 h-3 mr-1" />}
                {(condition !== 'New' && condition !== 'Like New') && <BarChart className="w-3 h-3 mr-1" />}
