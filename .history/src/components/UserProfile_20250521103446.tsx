...
export function UserProfile() {
  ...
  const handleToggleSoldStatus = async (listingId: string, currentStatus: boolean) => {    try {      // Make a more specific update with just the fields we need      const updateData = {         is_sold: !currentStatus        // updated_at will be set automatically by the trigger      };            const { error } = await supabase        .from('products')        .update(updateData)        .eq('id', listingId);              if (error) {        console.error('Error updating listing status:', error);        const errorMessage = error.message || 'Failed to update listing status';        setError(`Failed to update listing status: ${errorMessage}`);        return;      }            // Update the local state to reflect the change      setUserListings(userListings.map(listing =>         listing.id === listingId ? { ...listing, is_sold: !currentStatus } : listing      ));    } catch (err) {      console.error('Update error:', err);      setError('An unexpected error occurred. Please try again later.');    }  };
            ...
            <div className="p-6">
              ...
              {userListings.length === 0 ? (
                <div className="text-center py-16 px-4">
                  ...
                  <p className="text-gray-500 mb-6">Ready to sell something? Create your first listing now.</p>
                  <Link 
                    to="/add-product" 
