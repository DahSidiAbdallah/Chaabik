...
import { 
  ...
  Car, Building, Smartphone, Shirt, Home, Tool, DollarSign, MapPin, Hammer
} from 'lucide-react';
...
export function HomePage() {
  ...
  useEffect(() => {
    ...
    if (categoryParam) {
      ...
      if (category) {
        ...
        // Auto-open filters when category is selected
  ...
  // Database fetch with improved debugging
  useEffect(() => {
    async function fetchListings() {
      try {
        ...
        // First log available categories in database for debugging
        ...
        // If a category is selected, we need to correctly filter
        if (selectedCategory) {
          ...
          if (categoryObj) {
            ...
            console.log("Looking for subcategories of:", categoryObj.name);
            ...
            // Check if any of these subcategories exist in the database
            if (allProducts) {
              const matchingCategories = subcategoryIds.filter(id => 
                allProducts.some(p => p.category === id)
              );
              console.log("Matching subcategories in database:", matchingCategories);
            }
            
            // Use in() for filtering by multiple values
        ...
        if (error) {
          ...
          if (data.length === 0 && selectedCategory) {
            console.log("No products found for the selected category:", selectedCategory);
          }
          ...
          // If no products are found with database filters but we're filtering by category,
          // revert to showering static listings with category filter
          if (mappedProducts.length === 0 && selectedCategory) {
            console.log("Using static listings filtered by category");
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="flex-grow">
        ...
        {/* Filters Section - only visible when category selected */}
        {selectedCategory && (
          <section className="py-6 bg-white border-b border-gray-200">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900 flex items-center">
                  <span>{categoryName && t('categories.filteringCategory', { category: categoryName })}</span>
                  {isFiltersOpen ? null : (
                    <span className="ml-2 text-sm text-gray-500">
                      {priceRange.min || priceRange.max || locationFilter || conditionFilter ? 
                        t('filters.filtersApplied') : null}
                    </span>
                  )}
                </h3>
                <button
        ...
        {/* Listings Grid - improved debugging info */}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-8">
              ...
              <span className="text-sm font-normal text-gray-500 ml-2">
                {filteredListings.length > 0 ? `(${filteredListings.length} ${t('common.items')})` : ''}
              </span>
            </h2>
